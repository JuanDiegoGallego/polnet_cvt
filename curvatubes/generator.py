...
from curvatubes.utils import save_nii, slices, project_average
from curvatubes.filters import GeneralGaussianBlur3D_periodic, GeneralGaussianBlur3D_notperiodic
from curvatubes.filters import my_custom_GradHess
from curvatubes.energy import polykap_deg2, ratio_discr
...
def _generate_shape(params, tomo_shape, delta_x, xi, optim_method, optim_props,
                    ...
                    return_var=False, return_energy=False, check_viable=False, verbose=False, seed=None):
    ...
    # Set seed if None
    if seed is None:
        seed = hash(tuple(params)) % (2**32-1)
        np.random.seed(seed)
    print("Curvatubes seed: ", seed)
    ...
    if flow_type == 'cons':
        from curvatubes.filters import my_custom_Div
    ...
    if optim_method == 'bfgs':
        optimizer = torch.optim.LBFGS([variable], lr=learning_rate, max_iter=bfgs_max_iter, history_size=history_size,
                                      line_search_fn=line_search_fn)
    if optim_method == 'adam':
        optimizer = torch.optim.Adam([variable], lr=learning_rate, betas=betas_adam, eps=eps_adam,
    ...
    while nan_OK and viable_OK and n_evals <= maxeval:
        ...
        if display_all and optim_method == 'bfgs':
            if n_evals >= next_disp:
                ...
                print("at BFGS iteration {} and eval {},   umin = {}, umax = {}".format(iteration, n_evals,
